...
const translations = {
  'zh-CN': {
    ...
    notifications: {
      title: '邮件通知',
      items: ['每日任务摘要', '学生提交新材料提醒', '系统更新公告', '周报生成提醒']
    },
    security: {
      title: '修改密码',
      current: '当前密码',
  ...
  'en-US': {
    ...
    notifications: {
      ...
      items: ['Daily Task Summary', 'New Material Alerts', 'System Updates', 'Weekly Report Alerts']
    },
    security: {
               ...
               { id: 'security', label: t.tabs.security, icon: <Lock className="w-4 h-4"/> },
                ...
                <button
                   ...
                   className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                      ${activeTab === item.id 
                        ? 'bg-white dark:bg-zinc-800 text-primary-700 dark:text-primary-400 shadow-sm border border-gray-100 dark:border-white/5' 
                ...
                <div>
                   ...
                   <div className="space-y-4">
                      <div className="flex items-center justify-between py-3 border-b border-gray-100 dark:border-white/5">
                         ...
                         <select 
                            className="bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-white/10 rounded-lg text-sm px-3 py-1.5 outline-none focus:ring-2 focus:ring-primary-100 text-gray-800 dark:text-zinc-200 cursor-pointer"
                            value={language}
                ...
                <div>
                   ...
                   <div className="space-y-3">
                      {t.notifications.items.map((label, i) => (
                         <label key={i} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-zinc-800/50 rounded-lg cursor-pointer border border-transparent hover:border-gray-200 dark:hover:border-white/10">
                            <span className="text-sm text-gray-700 dark:text-zinc-300 font-medium">{label}</span>
                            <input type="checkbox" defaultChecked={i < 2} className="w-4 h-4 text-primary-600 rounded border-gray-300 dark:border-zinc-600 focus:ring-primary-500 dark:bg-zinc-700" />
                         </label>
                      ))}
                   </div>
                </div>
                ...
                <div>
                   ...
                   <div className="space-y-4 max-w-md">
                      <div>
                         ...
                         <input type="password" className="w-full px-3 py-2 bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-white/10 rounded-lg text-sm outline-none focus:border-primary-500 text-gray-800 dark:text-white focus:bg-white dark:focus:bg-zinc-900 transition-colors" />
                      </div>
                      <div>
                         <label className="block text-xs font-bold text-gray-500 dark:text-zinc-500 uppercase mb-1">{t.security.new}</label>
                         <input type="password" className="w-full px-3 py-2 bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-white/10 rounded-lg text-sm outline-none focus:border-primary-500 text-gray-800 dark:text-white focus:bg-white dark:focus:bg-zinc-900 transition-colors" />
                      </div>
                      <button className="px-4 py-2 bg-primary-600 text-white rounded-lg text-sm font-bold hover:bg-primary-700 shadow-sm flex items-center gap-2">
